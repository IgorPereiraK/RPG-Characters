...
export default function SkillBattle({
  name,
  image,
  magic,
  description,
  ...
  const borderColors: BorderColors = {
    ...
    "Transformação Vampírica": borderColorVladTransformacao,
    ...
    <div>
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger
            onClick={onClick}
            className="border-4 rounded-md border-gray-900 transition delay-75"
          ...
          <TooltipContent className="max-w-[400px] bg-gray-baldurSkill">
            ...
            <div className="flex px-2 py-1 gap-4 bg-gray-baldurSkill2">
              <div className="flex items-center text-gray-100">{type}</div>
              <div className="flex items-center text-gray-100">{mana}</div>
              {clickVladPassiva ? (
                clickVladPassiva !== 3 ? (
